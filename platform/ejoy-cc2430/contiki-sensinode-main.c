...
set_rime_addr(void)
{
  ...
  if(node_id == 0) {
    ...
    /* Switch to BANK3, map CODE: 0x8000 â€“ 0xFFFF to FLASH: 0x18000 â€“ 0x1FFFF */
    FMAP = 3;
    ...
    /* Remap 0x8000 â€“ 0xFFFF to BANK1 */
    FMAP = 1;
    ENABLE_INTERRUPTS();
#else
    /*
...
main(void)
{
  ...
  process_init();//½ø³Ì¹ÜÀí³õÊ¼»¯

  /* Init UART0
   * Based on the EJOY MCU CC2430 Circuit Design
   *  */
  uart0_init();//UART0´®¿Ú³õÊ¼»¯
...
//  putstring(SENSINODE_MODEL " (CC24");
  ...
  random_init(0);//Ëæ»úÊýÉú³ÉÆ÷³õÊ¼»¯

  /* start services */
  process_start(&etimer_process, NULL);//
  ctimer_init();//ctimer³õÊ¼»¯

  /* initialize the netstack */
  netstack_init();//ÍøÂçµ×²ãÕ»³õÊ¼»¯
